...
const SkillsPage = () => {
  ...
  const skills: Skill[] = [
    ...
    },
    {
      ...
      viewpoint: "Une communication efficace entre services est essentielle pour la réussite d'un projet. J'ai appris à écouter activement, à reformuler les demandes, à adapter mon discours selon l'interlocuteur et à documenter clairement les échanges. Cela permet d'éviter les malentendus, de gagner du temps et de construire une relation de confiance. J'accorde une importance particulière à la pédagogie, surtout quand il s'agit de faire comprendre des décisions techniques à des profils non-spécialistes.",
    ...
    },
    {
      ...
      viewpoint: "Windows et Active Directory restent des piliers dans la majorité des environnements informatiques professionnels. Être à l'aise avec leur administration, c'est pouvoir gérer efficacement les utilisateurs, sécuriser les postes et garantir un fonctionnement fluide du parc informatique. Ce sont des compétences que j'ai consolidées avec l'expérience, notamment en apprenant à adapter les configurations en fonction des contextes métier (bureautique, production, audit) pour gagner en fiabilité et en temps.",
  ...
  // Group skills by type for column layout
  const skillsByType = {
    generale: filteredSkills.filter(skill => skill.type === "generale"),
    technique: filteredSkills.filter(skill => skill.type === "technique"),
    comportementale: filteredSkills.filter(skill => skill.type === "comportementale")
  };
    ...
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <main className="flex-1 py-12">
        <div className="container mx-auto px-4">
          {!expandedSkill && (
            <>
              <h1 className="text-4xl md:text-5xl font-bold text-primary mb-6 text-center">Mes Compétences</h1>
          ...
          {expandedSkill ? (
            ...
            displayedSkills.map((skill) => (
              <div key={skill.id} id={skill.id}>
                ...
                <Card className="mb-8">
                  <CardHeader>
                    <CardTitle className="text-xl flex items-center gap-2">
                      💬 Mon point de vue sur la compétence
                    </CardTitle>
                  </CardHeader>
                  <CardContent>
                    <p className="text-gray-700">{skill.viewpoint}</p>
                  </CardContent>
                </Card>
            ...
            // Three-column layout based on skill type
            filter === "all" ? (
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                {/* Générales Column */}
                <div className="space-y-6">
                  <h2 className="text-2xl font-bold text-green-600 text-center mb-6">Générales</h2>
                  {skillsByType.generale.map((skill) => (
                    <Card 
                      key={skill.id} 
                      className="hover:shadow-md transition-all border border-green-200 cursor-pointer bg-green-50/30"
                      onClick={() => toggleSkillExpansion(skill.id)}
                    >
                      <CardHeader className="pb-3">
                        <CardTitle className="text-lg flex items-center gap-3 text-green-700">
                      ...
                      <CardContent className="pt-0">
                        ...
                        <div className="mt-4 flex flex-wrap gap-1">
                          {skill.relatedProjects.slice(0, 3).map(projectId => (
                ...
                {/* Techniques Column */}
                <div className="space-y-6">
                  <h2 className="text-2xl font-bold text-blue-600 text-center mb-6">Techniques</h2>
                  {skillsByType.technique.map((skill) => (
                    <Card 
                      key={skill.id} 
                      className="hover:shadow-md transition-all border border-blue-200 cursor-pointer bg-blue-50/30"
                      onClick={() => toggleSkillExpansion(skill.id)}
                    >
                      <CardHeader className="pb-3">
                ...
                {/* Comportementales Column */}
                <div className="space-y-6">
                  <h2 className="text-2xl font-bold text-purple-600 text-center mb-6">Comportementales</h2>
                  {skillsByType.comportementale.map((skill) => (
                    <Card 
                      key={skill.id} 
                      className="hover:shadow-md transition-all border border-purple-200 cursor-pointer bg-purple-50/30"
                      onClick={() => toggleSkillExpansion(skill.id)}
                    >
                      <CardHeader className="pb-3">
                        ...
                        <Badge variant="outline" className="text-purple-600 border-purple-600 w-fit">
              ...
              <div className="max-w-4xl mx-auto space-y-6">
                {displayedSkills.map((skill) => {
                  const colorClass = skill.type === "generale" ? "green" : skill.type === "technique" ? "blue" : "purple";
                  return (
                    <Card 
                      key={skill.id} 
                      className={`hover:shadow-md transition-all border border-${colorClass}-200 cursor-pointer bg-${colorClass}-50/30`}
                      onClick={() => toggleSkillExpansion(skill.id)}
                    >
                      <CardHeader className="pb-3">
                        <CardTitle className={`text-xl flex items-center gap-3 text-${colorClass}-700`}>
                          <span className="text-3xl">{skill.emoji}</span>
                          {skill.title}
                        </CardTitle>
                        <Badge variant="outline" className={`text-${colorClass}-600 border-${colorClass}-600 w-fit`}>
                          Compétence {skill.type === "generale" ? "générale" : skill.type}
                        </Badge>
                      </CardHeader>
                      <CardContent className="pt-0">
